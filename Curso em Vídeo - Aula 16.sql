...
/*Relacionamento entre tabelas: Chave Estrangeira e JOIN*/
desc cursosPrepara;
desc pessoasprepara;
/*Adiciona a coluna na tabela*/
alter table pessoasprepara add column cursoPreferido int;
/*Faz o relacionamento entre a coluna adicionada, dizendo que ela é uma chave estrangeira e referencia qual o valor de origem*/
alter table pessoasprepara add foreign key (cursopreferido) references cursosprepara(idCurso);
/*ADICIONANDO OS CURSOS PREFERIDOS*/
select * from pessoasprepara;
update pessoasprepara set cursoPreferido = '5' where idPessoas = '3';

/*Podemos realizar a junção entre as tabelas para que as informações se relacionem*/
/*Retorna o nome dos alunos e o nome dos seus cursos preferidos*/
select pessoasprepara.nome, cursosprepara.nomeCurso from pessoasprepara join cursosprepara on cursosprepara.idCurso = pessoasprepara.cursoPreferido;

/*AULA 16 - CRIANDO TABELA COM CHAVE ESTRANGEIRA*/
create table pessoasAssistemCursos (
	idAlunoAssisteCurso int not null auto_increment,
    dataInicio date,
    alunoAssistindo int,
    cursoAssistindo int,
    primary key (idAlunoAssisteCurso),
    foreign key (alunoAssistindo) references pessoasprepara(idPessoas),
    foreign key (cursoAssistindo) references cursosprepara(idCurso)
) default charset = utf8;

select * from pessoasAssistemCursos;
show tables;

insert into pessoasAssistemCursos values (default, '2022-08-10', 1, 5);
insert into pessoasAssistemCursos values (default, '2022-08-10', 60, 8);
insert into pessoasAssistemCursos values (default, '2022-08-10', 29, 12);
insert into pessoasAssistemCursos values (default, '2022-08-10', 22, 13);
insert into pessoasAssistemCursos values (default, '2022-08-10', 18, 30);
insert into pessoasAssistemCursos values (default, '2022-08-10', 9, 15);
insert into pessoasAssistemCursos values (default, '2022-08-10', 3, 22);
insert into pessoasAssistemCursos values (default, '2022-08-10', 36, 29);
insert into pessoasAssistemCursos values (default, '2022-08-10', 47, 4);
insert into pessoasAssistemCursos values (default, '2022-08-10', 55, 3);

select pessoasPrepara.nome, cursosPrepara.nomeCurso
from pessoasPrepara 
JOIN pessoasAssistemCursos ON pessoasPrepara.idPessoas = pessoasAssistemCursos.alunoAssistindo
JOIN cursosPrepara ON cursosPrepara.idCurso = pessoasAssistemCursos.cursoAssistindo;
